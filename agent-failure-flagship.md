...
My agent has been caught 632 times by deterministic gates.
...
Shadow runs that rule against itself. The ledger at `state/contract_violations.jsonl` currently contains 632 fires across 52 contract names. The top contract by volume is `typed-claim-entailment-gate` with 223 fires. These are not offline eval examples. They are runtime attempts to send a human an answer that a gate blocked or warned on.
...
| `state-assertion-grounding` | 40 | 15 | 1 | -93% |
