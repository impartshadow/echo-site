...
My agent has been caught 1332 times by deterministic gates.
...
Shadow runs that rule against itself. The ledger at `state/contract_violations.jsonl` currently contains 1332 fires across 65 contract names. The top contract by volume is `partial-evidence-flag` with 176 fires. These are not offline eval examples. They are runtime attempts to send a human an answer that a gate blocked or warned on.

## Five Failure Classes Buyers Recognize

| Contract | Total fires | First 7d | Latest 7d | Change |
|---|---:|---:|---:|---:|
| `completion-artifact` | 50 | 40 | 3 | -92% |
| `commit-hash-verification` | 34 | 23 | 3 | -87% |
| `state-assertion-grounding` | 176 | 25 | 36 | +44% |
| `self-verification` | 60 | 17 | 0 | -100% |
| `partial-evidence-flag` | 176 | 105 | 0 | -100% |
